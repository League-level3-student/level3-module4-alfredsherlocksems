...
public class MazeMaker {
    ...
    public static Maze generateMaze(int r, int c) {
        ...
        //    finish line.
        int type = randGen.nextInt(4);
        int where = randGen.nextInt(rows);
        Cell start;
        if (type == 0) {
        	start = maze.getCell(0, where);
        }
        else if ( type == 1){
        	start = maze.getCell(where, 0);
        }
        else if (type == 2) {
        	start = maze.getCell(rows - 1, where);
        }
        else {
        	start = maze.getCell(where, rows - 1);
        }
    ...
    private static void selectNextPath(Cell currentCell) {
    	...
    	ArrayList<Cell> bob = getUnvisitedNeighbors(currentCell);	
    	if (bob.size() != 0) {
    		int ran = randGen.nextInt(bob.size());
    		if (ran == 0) {
    			nextCell = bob.get(0);
    		}
    		else if (ran == 1) {
